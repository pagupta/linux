...
static struct vmem_altmap * setup_pmem_pfn(struct virtio_pmem *vpmem, struct resource *res,
...
static blk_qc_t virtio_pmem_make_request(struct request_queue *q, struct bio *bio)
{
	blk_status_t rc = 0;
	struct bio_vec bvec;
...
static int virtio_pmem_probe(struct virtio_device *vdev)
{
	...
	vdev->priv = vpmem = devm_kzalloc(&vdev->dev, sizeof(*vpmem), GFP_KERNEL);
	...
	if (!virtio_has_feature(vdev, VIRTIO_PMEM_PLUG)) {
        ...
        virtio_cread(vpmem->vdev, struct virtio_pmem_config, size, &vpmem->size);
        ...
        res = devm_request_mem_region(&vdev->dev, res_pfn.start, resource_size(&res_pfn), "virtio-pmem");
	...
	if (devm_add_action_or_reset(&vdev->dev, virtio_pmem_release_queue, q))
		return -ENOMEM;
	...
	/*if (IS_ENABLED(CONFIG_ZONE_DEVICE)) {
         ...
         */
	if (devm_add_action_or_reset(&vdev->dev, virtio_pmem_freeze_queue, q))
	return -ENOMEM;
	...
	virtio_device_ready(vdev);
        ...
        if (devm_add_action_or_reset(&vdev->dev, virtio_pmem_release_disk, vpmem))
              return -ENOMEM;
